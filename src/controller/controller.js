...
class class1 {
    ...
    try {
      ...
      if (User.length == 1) {
        ...
        if (User[0].VehicleDetail.length < 2) {
          ...
          var a = {
            message: "You Can Not Add More Vehicle In This Account",
        ...
        if (user.length == 0) {
          ...
          if (PhoneNumberCheckOfficial == +91) {
            ...
            async function convertPhoneNumber(Parameter1) {
              ...
              let stringWithoutDashes = await stringWithoutSpaces.replace(
                /-/g,
                ""
              );

              let cleanedNumber = await stringWithoutDashes
                .replace(/\D/g, "")
                .replace(/^(\+)?1/, "");
            ...
            var Otpmessage =
              "WEVALET: " +
              otp +
              " is your OTP for the login . This OTP is valid only for 2 min. By Apti Enterprises LLC";
      ...
      if (req.body.Fcm) {
        ...
        if (!User && !User2) {
          ...
          if (Passwordmatch) {
            ...
            if (User) {
              var updateuser = await Todo2.findOneAndUpdate(
                { UserName: LowerCaseUsername },
                { $set: { token: token, Fcm: req.body.Fcm } },
                { new: true }
              );
              await updateuser.save();
              ...
              var updateuser = await Todo8.findOneAndUpdate(
                { Username: LowerCaseUsername },
                { $set: { token: token, Fcm: req.body.Fcm } },
                { new: true }
              );
  ...
  static g = async (req, res) => {
    try {
      ...
      const daysOfWeek2 = Object.keys(obj).filter(
        (key) => key !== "_id" && key !== "__v"
      );
      ...
      if (!dayValetTimes) {
        var a = {
          error: "Waiting times not defined for this day.",
      ...
      if (req.files["valetTicket"] && req.files["carPicture"] && req.UserName) {
        ...
        if (User) {
          if (User.token == headerValue) {
            ...
            const carPictures = req.files["carPicture"].map(
              (file) => Ip + "/public/" + file.filename
            );
  ...
  static i = async (req, res) => {
    try {
      if (req.UserName) {
        ...
        if (User) {
          if (User.token == headerValue) {
            var ParkedCar11 = await Todo4.find({
              RegistrationNumber: req.body.RegistrationNumber,
              status: "",
            });
            if (ParkedCar11.length == 0) {
              ...
              if (UserData[0].VehicleDetail) {
                ...
                if (
                  UserData[0].VehicleDetail[0].RegistrationNumber ==
              ...
              var a = {
                message: "Please Upload Valet Tickiet Picture",
                status: `${HTTP.SUCCESS}`,
              };
              res.status(HTTP.SUCCESS).json(a);
            }
              ...
              async function fetchData() {
                try {
                  ...
                  if (response.status === 200) {
                    ...
                    FcmTokenUser[0].ActiveParkingUser = array1.filter(function (
                      item
                    ) {
        ...
        if (User) {
          if (User.token == headerValue) {
            ...
            if (FcmTokenUser[0].VehicleDetail) {
              if (
                FcmTokenUser[0].VehicleDetail[0].RegistrationNumber ==
                req.body.RegistrationNumber
              ) {
            ...
            if (ParkedStatus == "Parked") {
              async function myAsyncFunction() {
                ...
                if (req.body.UpdatedParklocation) {
                  ParkedCar[ParkedCar.length - 1].UpdatedParklocation =
                    req.body.UpdatedParklocation;
                } else {
                  await UpdatedParklocation.push(Parklocation);
                }
      ...
      if (ParkedCar.length !== 0) {
        ...
        if (User) {
          if (headerValue == User.token) {
            ...
            var FindAndUpdateMany = await Todo7.find({
              RegistrationNumber: req.body.RegistrationNumber,
            });
            FindAndUpdateMany[
              FindAndUpdateMany.length - 1
            ].NotificationRemainingTime = formattedFutureTime;
            ...
            if (ParkedCar[0].UserWaitTime.length == 1) {
              ...
              const futureDate22 = new Date(
                currentDate22.getTime() + ParkedCar[0].UserWaitTime[0] * 1000
              );
            ...
            async function fetchData() {
              try {
                ...
                if (response.status === 200) {
                  ...
                  var result2 = await Todo4.findOne({
                    RegistrationNumber: req.body.RegistrationNumber,
                    status: "Accepted",
                  });
                  ...
                  const futureTimeInSurat3 = currentTimeInSurat22.add(
                    result.message,
                    "minutes"
                  );
                  const formattedFutureTime3 = futureTimeInSurat3.format(
                    "YYYY-MM-DDTHH:mm:ss"
                  );
      ...
      if (ParkedCar.length !== 0) {
        ...
        if (User) {
          ...
          if (headerValue == User.token) {
            ...
            var a = {
              message: "Customer wait time updated successfully",
              status: `${HTTP.SUCCESS}`,
            };
            res.status(HTTP.SUCCESS).json(a);
          } else {
      ...
      if (ParkedCar.length !== 0) {
        if (ParkedCar[ParkedCar.length - 1].UserWaitTime.length == 0) {
          ...
          const originalDateString =
            ParkedCar[ParkedCar.length - 1].UserWaitTime[1];
          ...
          const currentDate = new Date();
          const inputDateStringOfficial2 = currentDate
            .toISOString()
            .slice(0, 19)
            .replace("T", " ");
  ...
  static q = async (req, res) => {
    try {
      ...
      if (!User) {
        ...
        if (Passwordmatch) {
          var signuptoken = await jwt.sign(
  ...
  static u = async (req, res) => {
    try {
      if (req.session.userid) {
        ...
        const response = {
          message: "Admin Panel User Account Create Successfully",
          status: HTTP.SUCCESS,
        };
        res.status(HTTP.SUCCESS).json(response);
      } else {
    ...
    try {
      if (req.session.userid) {
        ...
        for (var i = 0; i < User.length; i++) {
          var User2 = await Todo4.find({
            Parklocation: User[i].UnitName,
            status: "Parked",
          });
          var b = {
        ...
        if (req.params.id4 == "All") {
          ...
          for (var i = 0; i < User.length; i++) {
            ...
            for (var j = 0; j < User2.length; j++) {
              ...
              var year2 = dateObject.getFullYear();
              var month2 = (dateObject.getMonth() + 1)
                .toString()
                .padStart(2, "0");
              var day2 = dateObject.getDate().toString().padStart(2, "0");
            ...
            for (var j = 0; j < User22.length; j++) {
              ...
              const month2 = String(dateObject2.getMonth() + 1).padStart(
                2,
                "0"
              );
      ...
      if (PetLogic3 && req.body.latitude && req.body.longitude) {
        ...
        if (headerValue == PetLogic3.token) {
          ...
          transformedData.forEach((element) => {
            element.kilometer =
              geolib.getDistance(targetLocation, element.location) / 1000;
          });
        ...
        if (headerValue == PetLogic3.token) {
          ...
          transformedData.forEach((element) => {
            if (element.Country !== "INDIA") {
              element.kilometer =
                geolib.getDistance(targetLocation, element.location) / 1609.34;
  ...
  static J = async (req, res) => {
    try {
      if (req.UserName) {
        ...
        var BusinessData = await Todo2.findOne({
          UnitName: ValetData.BusinessUnitName,
        });

        var User = await Todo4.find({
          status: "Requested",
          Parklocation: BusinessData.UnitName,
        });
        ...
        for (var i = 0; i < User.length; i++) {
          ...
          if (
            User[i].RegistrationNumber ==
            UserOfficial[0].VehicleDetail[0].RegistrationNumber
          ) {
            var CarPictureofficial = await UserOfficial[0].VehicleDetail[0]
              .Picture;
            User[i].CarPictureofficial = CarPictureofficial;
          } else {
            var CarPictureofficial = await UserOfficial[0].VehicleDetail[1]
              .Picture;
            User[i].CarPictureofficial = CarPictureofficial;
          }
  ...
  static M = async (req, res) => {
    try {
      if (req.Phone) {
        ...
        var User2 = await Todo4.find({
          RegistrationNumber: { $in: filteredArray },
        });
    ...
    try {
      ...
      for (var i = 0; i < User.length; i++) {
        if (User[i].VehicleDetail) {
          if (User[i].VehicleDetail[0]) {
            if (
              User[i].VehicleDetail[0].RegistrationNumber ==
              req.body.RegistrationNumber
            ) {
    ...
    try {
      if (req.UserName) {
        ...
        if (User[0].token == headerValue) {
          ...
          var ParkedCar2 = await Todo4.find({
            RegistrationNumber: req.body.RegistrationNumber,
            status: "Accepted",
          });

          var AwsState = await Todo2.find({
            UnitName: ParkedCar2[0].Parklocation,
          });
          ...
          var a = {
            message: "Vehicle Deliver Picture Upload Sucessfully",
            status: `${HTTP.SUCCESS}`,
          };
          res.status(HTTP.SUCCESS).json(a);
        } else {
    ...
    try {
      if (req.UserName) {
        ...
        if (User[0].token == headerValue) {
          ...
          const ParkedCar2 = await Todo4.findOneAndUpdate(
            { RegistrationNumber: registerNumber, status: "Accepted" },
            {
              $set: {
          ...
          async function fetchData() {
            try {
              ...
              if (response.status === 200) {
                ...
                axios
                  .post(`${Ip}/StatusChange2`, postData2)
                  .then((response) => {
                    const message = {
                      notification: {
  ...
  static S = async (req, res) => {
    try {
      ...
      if (ParkedCar.length !== 0) {
              ...
              .then((response) => {
                ...
                fcm
                  ...
                  .catch((error) => {
                    ...
                    var a = {
                      message:
                        "Request Cancle Sucessfully & Notification Does Not Send",
                      status: `${HTTP.INTERNAL_SERVER_ERROR}`,
                    };
                    res.status(HTTP.INTERNAL_SERVER_ERROR).json(a);
                  });
        ...
        axios
          .post(`${Ip}/RequestCancleByCustomer2`, postData)
          .then((response) => {
            var a = {
  ...
  static T = async (req, res) => {
    try {
      ...
      if (ParkedCar.length !== 0) {
        ...
        var a = {
          message: "Does Not Find Any Request",
          status: `${HTTP.NOT_FOUND}`,
        };
        res.status(HTTP.SUCCESS).json(a);
      }
      ...
      if (req.body.otp && req.body.Phone) {
        ...
        if (User2) {
          if (User2.ForgetPasswordOtp == req.body.otp) {
            var a = {
              message: "Password Forget Sucessfully",
              status: `${HTTP.SUCCESS}`,
            };
            res.status(HTTP.SUCCESS).json(a);
          } else {
...
class class2 {
      ...
      if (req.body.Phone && req.Phone && req.body.Name) {
        ...
        if (User2[0].token == headerValue) {
          if (User2[0].Member.length < 5) {
            ...
            var a = {
              message: "You Can Not Add More Member In This Account",
              status: `${HTTP.FORBIDDEN}`,
            };
            res.status(HTTP.FORBIDDEN).json(a);
          }
      ...
      if (
        ...
        if (headerValue == User[0].token) {
          ...
          if (!User1 && !User2) {
            ...
            const response = {
              message: "Please Choose Another Phone Number",
              status: HTTP.UNAUTHORIZED,
            };
            res.status(HTTP.UNAUTHORIZED).json(response);
          }
      ...
      if (req.UserName && req.body.UserName) {
        ...
        if (User) {
          if (headerValue == User.token) {
            ...
            var message2 = {
              message: "Valet Delete Successfully",
              status: `${HTTP.SUCCESS}`,
            };
            res.status(HTTP.SUCCESS).json(message2);
          } else {
      ...
      if (req.UserName) {
        ...
        if (User) {
          if (headerValue == User.token) {
            ...
            if (User.UserName == User2[0].BusinessManagerUserName) {
              for (var i = 0; i < req.files.length; i++) {
                await User2[0].Profile.unshift(
                  Ip + "/public/" + req.files[i].filename
                );
              }
              ...
              if (req.body.Username || req.body.Phone) {
                if (req.body.Username && req.body.Phone) {
                  ...
                  if (!User3 && !User4 && !User5 && !User6) {
                    ...
                    var message2 = {
                      message: "Valet Update Successfully",
                      status: `${HTTP.SUCCESS}`,
                    };
                    res.status(HTTP.SUCCESS).json(message2);
                  } else {
                    if ((User3 || User4) && (User5 || User6)) {
                      var message2 = {
                        message:
                          "Please Choose Another Phone Number And Username",
              ...
              var message2 = {
                message: "Please Choose Another Token",
                status: `${HTTP.NOT_ALLOWED}`,
              };
              res.status(HTTP.NOT_ALLOWED).json(message2);
            }
  ...
  static m = async (req, res) => {
    try {
      ...
      if (PetLogic3) {
        ...
        if (headerValue == PetLogic3.token) {
          var User2 = await Todo7.find({
            UserName: PetLogic3.UserName,
            ParkInTime: { $ne: "" },
          });
      ...
      if (req.UserName) {
        ...
        if (User) {
          if (headerValue == User.token) {
            ...
            const SendData2 = await SendData.sort(
    ...
    try {
      if (req.UserName) {
        ...
        if (User) {
          if (headerValue == User.token) {
            var data = await [
              User.UserWaitTime[0].Monday[0][1],
              User.UserWaitTime[0].Tuesday[0][1],
              User.UserWaitTime[0].Wednesday[0][1],
              User.UserWaitTime[0].Thursday[0][1],
              User.UserWaitTime[0].Friday[0][1],
              User.UserWaitTime[0].Saturday[0][1],
              User.UserWaitTime[0].Sunday[0][1],
            ];
            var message2 = {
              message: "Data Load Successfully",
              data: data,
      ...
      if (
        ...
        if (User) {
          if (headerValue == User.token) {
            var UserWaitTimeArray = [
              {
                Monday: [["", req.body.Day1]],
                Tuesday: [["", req.body.Day2]],
                Wednesday: [["", req.body.Day3]],
                Thursday: [["", req.body.Day4]],
                Friday: [["", req.body.Day5]],
                Saturday: [["", req.body.Day6]],
                Sunday: [["", req.body.Day7]],
              },
            ];
      ...
      if (req.UserName) {
        ...
        if (User) {
          if (headerValue == User.token) {
            ...
            if (User2.length !== 0) {
              var User6 = await Todo.find({
                UserName: User2[User2.length - 1].Member[0],
              });

              if (
                User6[User6.length - 1].VehicleDetail[0].RegistrationNumber ==
                User2[User2.length - 1].RegistrationNumber
              ) {
            ...
            if (User3.length !== 0) {
              ...
              for (var i = 0; i < User3.length; i++) {
                ...
                if (
                  User33[0].VehicleDetail[0].RegistrationNumber ==
                  User3[i].RegistrationNumber
                ) {
              ...
              const SendData3 = await SendData4.sort(
                (a, b) => b.Request - a.Request
              );
  ...
  static C = async (req, res) => {
    try {
      if (req.session.userid) {
        ...
        const response = {
          message: "Payment Related Data Update Successfully",
          status: HTTP.SUCCESS,
        };
        res.status(HTTP.SUCCESS).json(response);
      } else {
      ...
      for (var i = 0; i < User.length; i++) {
        if (User[i].VehicleDetail) {
          ...
          const formattedFutureTime =
            futureTimeInSurat.format("YYYY-MM-DDTHH:mm");
    ...
    try {
      ...
      for (var i = 0; i < User.length; i++) {
        if (User[i].VehicleDetail) {
          ...
          if (User[i].VehicleDetail[0]) {
            if (
              User[i].VehicleDetail[0].status == "Deliver" &&
              User[i].VehicleDetail[0].StatusChange == currentTimeInSurat
            ) {
          ...
          if (User[i].VehicleDetail[1]) {
            if (
              User[i].VehicleDetail[1].status == "Deliver" &&
              User[i].VehicleDetail[1].StatusChange == currentTimeInSurat
            ) {
  ...
  static F = async (req, res) => {
    try {
      ...
      var User = await Todo7.find({
        ParkInTime: "",
        NotificationRemainingTime: currentTimeInSurat2,
      });
      ...
      if (User.Phone) {
        ...
        if (User2) {
          ...
          if (DeleteStatus == 0) {
            ...
            var a = {
              message: "Customer Does Not Delete",
              status: `${HTTP.UNAUTHORIZED}`,
            };
            res.status(HTTP.UNAUTHORIZED).json(a);
          }
...
class QRCodeClass {
  ...
  //       const otp = generateOTP();
  //       const qrUrl = `${QRBaseUrl}?token=${tokenNumber}&businessName=${User._id}`;
  //       const qrCodeBuffer = await generateQRCode(qrUrl); // Get QR code as buffer

  //       const newCar = new HotelQrCode({
  //         tokenNumber,
  //         qrCode: `${Ip}/qr_codes/${tokenNumber}`, // just for reference
  //         businessId: User._id,
  //         otp,
  //       });
  ...
  //       // Add QR code and token number to PDF
  //       doc.fontSize(12).text(`Token Number: ${tokenNumber}`, {
  //         align: "center",
  //       });
  ...
  //       // Add new page for the next QR code
  //       if (i < endNumber) {
  //         doc.addPage();
  //       }
  //     }
  ...
  static GenerateQRCode = async (req, res) => {
    try {
      ...
      const uploadParams = {
        Bucket: process.env.AWS_S3_BUCKET,
        Key: `pdfs/${pdfFileName}`,
        Body: doc, // Pass the PDF document stream
        ACL: "public-read",
        ContentType: "application/pdf",
      };

      // Loop through each number to generate QR codes and add to PDF
      for (let i = startNumber; i <= endNumber; i++) {
        const tokenNumber = i.toString();
        ...
        // Add QR code and token number to the PDF
        doc
          .fontSize(12)
      ...
      // End the PDF document stream (triggers S3 upload)
      doc.end();
      ...
      s3.upload(uploadParams, (err, data) => {
        ...
        res.status(HTTP.SUCCESS).json({
          message: "QR codes generated successfully!",
          pdfUrl, // Return the S3 PDF URL for download
          status: `${HTTP.SUCCESS}`,
        });
      });
    } catch (error) {
