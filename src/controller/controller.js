      ...
      if ((PetLogic3 || !req.isAuthenticated) && (req.body.latitude || req.body.longitude)) {
        ...
        if (!req.isAuthenticated || (PetLogic3 && headerValue === PetLogic3.token)) {
          ...
          const transformedData = data.map((item) => ({
            ...
            City: item.City,
            State: item.State,
            // FullAddress: `${item.UnitAddress}, ${item.Zip}`,
            Profile: item.Profile,
            Rating: item.Rating,
            Review: item.Review,
...
class QRCodeClass {
  ...
  static AcceptCar = async (req, res) => {
    try {
      if (req.UserName) {
        const headerValue = req.get("Authorization");
        var User = await Todo8.findOne({ Username: req.UserName });
  ...
  static AcceptedCarDetails = async (req, res) => {
    try {
      if (req.UserName) {
        const headerValue = req.get("Authorization");
        var User = await Todo8.findOne({ Username: req.UserName });
...
class UserService {
  static DeleteUser = async (req, res) => {
    try {
      ...
      if (!user) {
        var a = { message: "Account Not Exist", status: `${HTTP.NOT_FOUND}` };
        return res.status(HTTP.NOT_FOUND).json(a);
      }

      const userDelete = await Todo.findOneAndDelete({ Phone: req.Phone });
      if (userDelete) {
